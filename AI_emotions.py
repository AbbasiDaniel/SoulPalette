...
# ==========================================
# 3. Inference Function (برای فراخوانی در پروژه اصلی)
# ==========================================
_MODEL = None
_DEVICE = None
...
def tell_emotion(face):
    ...
    # بارگذاری تنبل (Lazy Loading) مدل در اولین فراخوانی
    if _MODEL is None:
        _DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        _MODEL = FastEmotionCNN(num_classes=NUM_CLASSES)
        _MODEL.load_state_dict(torch.load(MODEL_SAVE_PATH, map_location=_DEVICE, weights_only=True))
        _MODEL.to(_DEVICE)
        _MODEL.eval()

    # رفع مشکل غیرپیوسته بودن حافظه برش OpenCV
    face = np.ascontiguousarray(face)

    tensor = torch.from_numpy(face).float()
    tensor = tensor.permute(2, 0, 1)                  # تبدیل به (C, H, W)
    tensor = tensor.mean(dim=0, keepdim=True)         # تبدیل BGR به Grayscale
    tensor = tensor.unsqueeze(0)                      # تبدیل به (1, 1, H, W)
    tensor = F.interpolate(tensor, size=(IMAGE_SIZE, IMAGE_SIZE), mode='bilinear', align_corners=False)
    tensor = (tensor / 255.0 - 0.5) / 0.5             # نرمالسازی به [-1, 1]
...
# ==========================================
# 4. Training Loop (فقط هنگام اجرای مستقیم فایل)
# ==========================================
if __name__ == '__main__':
    # تعریف Transformها و Datasetها داخل Guard قرار گرفت تا موقع Import خطایی رخ ندهد
